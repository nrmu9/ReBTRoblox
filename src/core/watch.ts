...
const makeWatcher = (targetPromise: Promise<any>, finishPromise: Promise<any> | null, parent: Watcher | null): Watcher => ({
	targetPromise,
	finishPromise,
	parent,
	...
	$then(callback) {
		...
		if(callback) { void next.targetPromise.then(callback) }
		return next
	},

	$back() {
		if(!this.parent) { throw new Error("Cannot call $back on a top level watcher") }
		return this.parent
	},
...
export const watch = (
	...
	if(props?.continuous) {
		...
		dispose = hookWatch(list[0], element => {
			...
			catch(err) { console.error("[btr] watch callback failed", err) }
		}, { root })
	} else {
...
export const onRemove = (target: Node, callback: () => void) => {
	if(!document.contains(target)) { return callback() }
