...
export const setCurrentPage = (page: any): void => { currentPage = page }
...
export const getCurrentPage = (): any => {
	for(const [name, page] of Object.entries(PAGE_INFO) as [string, any][]) {
		...
		if(!domainMatches.includes(location.hostname)) { continue }
		...
		for(let pattern of page.matches) {
			...
			if(matches) { return { ...page, name, matches: matches.slice(1) } }
		}
	}
