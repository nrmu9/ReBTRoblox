...
import { RBXAnimator } from "@/rbx/Avatar/Animator"
import { RBXAvatarRigs } from "@/rbx/Avatar/AvatarRigs"
...
export const RBXAvatar = (() => {
	...
	function applyMesh(obj: any, mesh: Mesh) {
		...
		if (obj instanceof THREE.SkinnedMesh) {
			...
			if (mesh.bones && mesh.skinIndices && mesh.skinWeights) {
				...
				const boneCount: Record<string, any> = {}
				...
				obj.skeleton = new THREE.Skeleton(
					bones,
					bones.map((x) => x.inverse),
				)
	...
	class AvatarAsset extends EventEmitter {
		...
		get(...args: any[]) {
			...
			if (
				BodyAssetTypeIds.includes(this.assetTypeId) ||
				AccessoryAssetTypeIds.includes(this.assetTypeId)
			) {
			...
			AssetCache.loadModel(this.id, state.request, (model: any) => {
				...
				switch (this.assetTypeId) {
					...
					case AssetType.RightLeg: {
						const R15Folders: any[] = []

						for (const child of model) {
							if (child.ClassName === "SpecialMesh" && state.playerType === "R6") {
								if (
									this.assetTypeId === AssetType.Head ||
									this.assetTypeId === AssetType.DynamicHead
								) {
								...
								if (child.Name === "R6" && state.playerType === "R6") {
									...
									R15FolderPriority.includes(child.Name) &&
									state.playerType === "R15"
								) {
						...
						if (R15Folders.length > 0) {
							R15Folders.sort(
								(a, b) =>
									R15FolderPriority.indexOf(a.Name) - R15FolderPriority.indexOf(b.Name),
							)
		...
		loadHeadR6(state: any, mesh: any) {
			...
			for (const inst of mesh.Children) {
				if (inst.ClassName !== "Vector3Value" || !inst.Name.endsWith("Attachment")) {
					continue
				}
		...
		loadBodyPartsR6(state: any, charmeshes: any) {
			for (const charmesh of charmeshes) {
				...
				state.bodyparts.push(<Record<string, any>>{
					...
					meshId: this.validateAndPreload(
						"Mesh",
						charmesh.MeshId ? AssetCache.toAssetUrl(charmesh.MeshId) : null,
					),
					texId: this.validateAndPreload(
						"Image",
						charmesh.OverlayTextureId ? AssetCache.toAssetUrl(charmesh.OverlayTextureId) : null,
					),

					baseTexId: this.validateAndPreload(
						"Image",
						charmesh.BaseTextureId ? AssetCache.toAssetUrl(charmesh.BaseTextureId) : null,
					),
				})
			}
		}
	...
	class Avatar extends EventEmitter {
		...
		init() {
			if (this.hasInit) {
				throw new Error("Avatar has already been initialized")
			}
			this.hasInit = true
			...
			for (const name of R15BodyPartNames) {
				const limb = name
					.replace(/\s|Lower|Upper/g, "")
					.replace(/Hand|Foot/, (x: string): string => ({ Hand: "Arm", Foot: "Leg" })[x] ?? x ?? x)
					.replace(/^./, (x) => x.toLowerCase())
				const composite = composites[limb]

				sources[name] = new MergeSource()
				textures[name] = new MergeTexture(
					composite.canvas.width,
					composite.canvas.height,
					sources.bodyColors[limb],
					composite,
					sources[name],
				)
			}
		...
		updateBones(obj: any, acc?: any) {
			...
			for (const joint of this.sortedJointsArray) {
				if (isLayeredAccessory) {
					...
					joint.part1.skinnedPoseMatrix.multiplyMatrices(
						joint.skinnedPoseMatrix,
						joint.bakedC1Inverse,
					)
				} else {
					if (obj.rbxBodypart) {
					...
					joint.skinnedNoScale
						.multiplyMatrices(
							joint.part0.skinnedNoScale,
							scalePosition(tempMatrix.copy(joint.C0), obj.rbxScaleMod),
						)
						.multiply(joint.transform)

					joint.part1.skinnedNoScale.multiplyMatrices(
						joint.skinnedNoScale,
						scalePosition(tempMatrix.copy(joint.C1Inverse), obj.rbxScaleMod),
					)
				}
			}
			...
			for (const bone of obj.rbxBones) {
				...
				if (isLayeredAccessory) {
					...
					bone.inverse
						.copy(obj.skinnedPoseMatrix)
						.invert()
						.multiply(joint.skinnedPoseMatrix)
						.invert()
		...
		update() {
			...
			rootPart?.matrixNoScale
				.makeRotationFromEuler(this.offsetRot)
				.setPosition(0, this.hipHeight + (rootPart?.rbxSize[1] ?? 0) / 2, 0)

			// Update joints
			for (const joint of this.sortedJointsArray) {
				const transform = this.animationsDisabled
					? null
					: this.animator.getJointTransform(joint.part0.name, joint.part1.name)
			...
			// Update composites
			const activeComposites =
				this.playerType === "R6"
					? [this.composites.r6]
					: [
							this.composites.torso,
							this.composites.leftArm,
							this.composites.rightArm,
							this.composites.leftLeg,
							this.composites.rightLeg,
						]
		...
		getScaleMod(partName: any, scaleType?: any, partScaleType?: any, result?: any) {
			// Only use partScaleType with accessories! C:
			if (!result) {
			...
			if (scaleType === "ProportionsNormal") {
				...
				if (
					partScaleType &&
					partScaleType !== "ProportionsNormal" &&
					partScaleType !== "ProportionsSlender"
				) {
				...
				endScale
					.set(1, 1, 1)
					.divide(propMod)
					.lerp(tempVector.set(1, 1, 1), this.scales.proportion)
				...
				endScale
					.copy(defaultMod)
					.multiply(tempVector.set(1, 1, 1).lerp(propMod, this.scales.proportion))

				if (
					partScaleType &&
					(partScaleType === "ProportionsNormal" || partScaleType === "ProportionsSlender")
				) {
		...
		_refreshBodyParts() {
			...
			const attachmentOverride: Record<string, any> = {}
			const clothingOverride: Record<string, any> = {}
			const bodypartOverride: Record<string, any> = {}
			...
			assets.sort((a, b) =>
				a.priority === b.priority ? a.lastIndex - b.lastIndex : a.priority - b.priority,
			)
			...
			for (const acc of Object.values(accessories) as any[]) {
				...
				if (!acc.obj) {
					...
					if (acc.pbrEnabled) {
						...
						material = new THREE.MeshStandardMaterial({
							...
							opacity: opacity,
							map: MergeSource.fromRGB(
								...(<[number, number, number]>acc.baseColor),
							).toTexture(),
						})
				...
				if (attachment) {
					acc.bakedCFrame
						.copy(attachment.bakedCFrame)
						.multiply(scalePosition(tempMatrix.copy(attCFrame), acc.obj.rbxScaleMod))
				} else {
				...
				if (acc.offset) {
					acc.bakedCFrame.multiply(
						tempMatrix.makeTranslation(...(<[number, number, number]>acc.offset)),
					)
				}
		...
		getLayeredRequest() {
			...
			for (const acc of this.accessories) {
				if (acc.wrapLayer) {
					if (invalidLayeredAssetIds[acc.asset.id]) {
						continue
					}

					if (acc.obj.rbxMeshLoading) {
						// Don't request layered stuff if all meshes are not loaded yet
						return false
					}

					if (acc.wrapLayer?.autoSkin && !acc.obj.visible) {
						// Don't request hidden autoskinned accessories
						continue
					}
			...
			for (const part of Object.values(this.parts)) {
				...
				if (bodypart.asset) {
					if (invalidLayeredAssetIds[bodypart.asset.id]) {
						continue
					}

					if (part.rbxMeshLoading) {
						// Don't request layered stuff if mesh is not loaded yet
						return false
					}

					if (!bodyparts.find((x) => x.id === bodypart.asset.id)) {
						// Multiple parts can share the same asset
		...
		_refreshLayeredClothing() {
			...
			if (request.hash === this.layeredCurrentRequest?.hash) {
				return
			}
			if (request.hash === this.layeredFailedRequest?.hash) {
				return
			}
			...
			this._fetchLayeredClothing(request).then((result) => {
				if (this.layeredCurrentRequest !== request) {
					return
				}
				this.layeredCurrentRequest = null
		...
		async _fetchLayeredClothing(request: any) {
			...
			if (!objHash) {
				...
				while (true) {
					...
					if (!json || request !== this.layeredCurrentRequest) {
						return
					}
					...
					if (json.state === "Pending") {
						...
						if (request !== this.layeredCurrentRequest) {
							return
						}
						continue
					}
				...
				if (objHash) {
					btrLocalStorage.setItem(`btrLayeredCache-${request.hash}`, objHash, {
			...
			for (const line of lines) {
				if (line.startsWith("g ")) {
					...
					group = {
						...
						faces: [],
						normals: [],
						vertexStartIndex: vertexCounter,
					}
					groups.push(group)
			...
			const anchor = groups.find(
				(x) =>
					x.name.startsWith("Handle") &&
					x.faces.length === 36 &&
					hashString(JSON.stringify([x.faces, x.uvs])) === "4734F03A",
			)
			if (!anchor) {
				return console.log("Found no anchor")
			}

			groups.splice(groups.indexOf(anchor), 1)

			const attachment = this.attachments.HatAttachment
			const scaleMod = this.getScaleMod(
				attachment.parent.name,
				"Classic",
				attachment.parent.rbxScaleType,
			)
			const bakedCFrame = scalePosition(
			...
			const anchorMatrix = attachment.parent.layeredMatrix
				.clone()
				.multiply(attachment.bakedCFrame)
				.multiply(bakedCFrame)
			...
			const inverseRenderMatrix = new THREE.Matrix4()
				.makeTranslation((max[0] + min[0]) / 2, (max[1] + min[1]) / 2, (max[2] + min[2]) / 2)
				.invert()
			...
			const getGroupMatchPartial = (mesh: Mesh, group: any, box: UVBox) => {
				...
				outer: for (let i = 0, len = group.uvs.length; i < len; i += 2) {
					const u = (group.uvs[i] - x0) / width
					const v = (group.uvs[i + 1] - y0) / height
			...
			for (const part of Object.values(this.parts)) {
				...
				for (const group of playerGroups) {
					const mapping =
						(partBox && getGroupMatchPartial(mesh, group, partBox)) ||
						getGroupMatchPartial(mesh, group, uvBoxes.full)
				...
				part.rbxLayered = {
					wrapTarget: part.rbxBodypart.wrapTarget || part.rbxDefaultBodypart.wrapTarget,
				}
			}
			...
			for (const acc of this.accessories) {
				...
				if (group) {
					...
					if (doAutoSkinHack) {
						...
						acc.obj.geometry.setAttribute(
							"uv",
							new THREE.BufferAttribute(Float32Array.from(group.uvs), 2),
						)
					}
