...
export const RBXAnimator = (() => {
	...
	class Animator {
		...
		play(anim, params) {
			...
			if (params?.onstop) {
				info.onstop = params.onstop
			}
			if (params?.onloop) {
				info.onloop = params.onloop
			}
		...
		update() {
			...
			for (const info of this.animations) {
				const weight =
					Math.max(0, Math.min(1, info.weight)) *
				...
				for (const [name, keyframes] of Object.entries(info.anim.keyframes) as [string, any][]) {
					...
					if (
						transform &&
						(transform.weight >= 1 ||
							(transform.working.weight >= 1 && transform.working.priority !== info.priority))
					) {
						continue
					}
					...
					if (transform.working.priority !== info.priority) {
						if (transform.working.weight > 0) {
							const newWeight =
								1 - (1 - transform.weight) * (1 - Math.min(1, transform.working.weight))
					...
					if (info.anim.isCurveAnimation) {
						...
						const set = (input, output) => {
							for (const [key, keyframes] of Object.entries(input) as [string, any][]) {
								...
								const prev =
									index === -1 ? keyframes[keyframes.length - 1] : keyframes[index - 1]
								const next = index === -1 ? null : keyframes[index]

								if (
									!next ||
									info.timePosition === prev.time ||
									prev.interpolationMode === 0
								) {
						...
						tempQuat.set(
							...(<[number, number, number, number]>(
								eulerToQuat(rot.x, rot.y, rot.z, keyframes.rotationOrder)
							)),
						)
					} else {
						const index = keyframes.findIndex((x) => x.time > info.timePosition)
						...
						if (!next || info.timePosition === prev.time) {
							...
							const easing = (EasingStyles[prev.easingstyle] || EasingStyles[0])[
								prev.easingdir || 0
							]
							const theta = easing((info.timePosition - prev.time) / (next.time - prev.time))

							tempPos
								.set(...(<[number, number, number]>prev.pos))
								.lerp(tempPos2.set(...(<[number, number, number]>next.pos)), theta)
							tempQuat
								.set(...(<[number, number, number, number]>prev.rot))
								.slerp(tempQuat2.set(...(<[number, number, number, number]>next.rot)), theta)
						}
					}
