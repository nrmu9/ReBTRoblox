			...
			template.$req(".redraw-avatar").after(
				html` <div class="btr-avatar-refinement-container">
					<div
						...
						ng-if="item.meta.position"
						ng-init="item.btrScale=item.meta.scale.X"
					>
						<div class="scale-container">
							<div
								class="text-label font-subheader-1"
								style="width:100%"
							...
							<div class="scale-label font-body">
								({{item.meta.position.X}}, {{item.meta.position.Y}}, {{item.meta.position.Z}})
							</div>
							...
							<div class="scale-label font-body">
								({{item.meta.rotation.X}}, {{item.meta.rotation.Y}}, {{item.meta.rotation.Z}})
							</div>
							<div style="width:100%;clear:both">
								<input
									type="range"
							...
							try {
								...
								const updateWearingAssets = () => {
									...
									for (const item of wearingAssets) {
										if (
											avatarRules.accessoryRefinementTypes.includes(item.assetType.id)
										) {
											if (!item.meta) {
												item.meta = { version: 1 }
											}
