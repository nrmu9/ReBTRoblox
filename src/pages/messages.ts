...
pageInit.messages = () => {
	...
	modifyAngularTemplate("messages-nav", (template) => {
		...
		if (curPage) {
			...
			const pageInput = html<HTMLInputElement>`<input
				type="text"
				ng-keydown="$event.which===13&&$event.target.blur()"
				ng-blur="btr_setPage($event)"
			/>`
		...
		if (markAsUnread) {
			markAsUnread.after(
				" ",
				html<HTMLButtonElement>`<button class="btn-control-sm btr-markAllAsReadInbox">
					Mark All As Read
				</button>`,
			)
		} else {
