...
const routes = {
	"GET /poll"(req, res) {
		...
		if (jobs.length) {
			return reply(res, 200, jobs.shift())
		}
	...
	"GET /logs"(req, res, url) {
		...
		if (level) {
			out = out.filter((entry) => entry.level === level)
		}
		if (match) {
			out = out.filter((entry) => JSON.stringify(entry).includes(match))
		}
...
const server = http.createServer(async (req, res) => {
	...
	if (!route) {
		return reply(res, 404, { error: "unknown route", routes: Object.keys(routes) })
	}
	...
	try {
		...
		if (!res.headersSent) {
			reply(res, 500, { error: String((err && err.stack) || err) })
		}
	}
})
